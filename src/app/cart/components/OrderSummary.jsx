...
function calculateAverageDiscount() {
  ...
  <span className='text-red-600'>-<CurrencyFormatter value={cartItems.reduce((acc, item) => acc + (item.price.old - item.price.new) * item.quantity, 0)} /></span>
</div>
        ...
        <div className="flex justify-between text-[16px] font-semibold">
          ...
          <span className='text-[20px]'><CurrencyFormatter value={calculateTotal()} /></span>
        </div>
      </div>
      <div className="flex items-center gap-2 mt-6">
        <div className="flex items-center flex-1 gap-2 px-4 py-2 bg-gray-100 rounded-full">
          ...
          <input type="text" placeholder="Add promo code" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} className="flex-1 text-sm bg-transparent outline-none" />
        </div>
      ...
      <button className="flex items-center justify-center w-full gap-2 py-3 mt-6 text-white transition bg-black rounded-full hover:opacity-90"> Go to Checkout <ArrowRight size={16} /> </button>
    </div>
  );
}
