...
const categories = [
  {
    label: "Casual",
    image: "/assets/images/categories/image 11.png",
  },
  {
    label: "Formal",
    image: "/assets/images/categories/image 12.png",
  },
  {
    label: "Party",
    image: "/assets/images/categories/image 13.png",
  },
  {
    label: "Gym",
    image: "/assets/images/categories/image 14.png",
  },
];
        ...
        <div className="hidden max-w-[1110px] grid-cols-10 gap-[20px] mt-15 mb-15 mx-auto md:grid">
          {categories.map((cat, index) => (
            <Link
              ...
              key={index}
              className={`block ${index === 0 || index === 3 ? "col-span-3" : "col-span-7"}`}
            >
              <div className="relative overflow-hidden bg-white shadow-xs rounded-xl h-[290px]">
                ...
                />
                <div className="absolute left-0 p-4 text-[36px] font-bold text-black top-2">{cat.label}</div>
              </div>
            </Link>
          ))}
        </div>
            ...
            <Link href={`/${cat.label.toLowerCase()}`} key={index}>
              <div className="relative overflow-hidden mb-5 bg-white shadow-sm h-[190px] rounded-xl">
                <Image
                  ...
                  sizes="(max-width: 1200px) 100vw, 1200px"
                />
                <div className="absolute left-0 p-4 text-[32px] font-bold text-black top-2">{cat.label}</div>
              </div>
            </Link>
          ))}
        </div>
