...
const SearchBarDisplay = ({ setSearchItem, recentSearches, searchResults }: SearchBarDisplayProps) => {
  return (
      ...
      <ul>
        {recentSearches.map((search: string, index: number) => (
          <li key={index} className="py-2 border-b border-gray-200">
            <button className="text-sm text-gray-600" onClick={() => setSearchItem(search)}>
              {search}
            </button>
          </li>
        ))}
